    ...
    <div class="row justify-content-center">
      <div class="col-md-8 toastify-container">
        ...
        <div class="alert alert-success" role="alert" v-if="success">
          <strong>Store saved!</strong> Please check your email for confirmation.
        </div>

        <div class="alert alert-danger" role="alert" v-if="error"><strong>Registration failed!</strong> Please try again later.</div>

        <div class="alert alert-danger" role="alert" v-if="errorShopExists">
          <strong>Shop already exists</strong> Please try another one.
        </div>
      </div>
    </div>
    <div class="row justify-content-center">
      <div class="col-md-8">
        <form id="addstore-form" name="addstoreForm" role="form" v-on:submit.prevent="addstore()" v-if="!success" no-validate>
          <div class="form-group">
            <label class="form-control-label" for="shopname">Shop Name</label>
            <input
              type="text"
              class="form-control"
              ...
              v-bind:placeholder="$t('global.form[\'shopname.placeholder\']')"
              data-cy="addstore"
            />
            <div v-if="$v.registerAccount.login.$anyDirty && $v.registerAccount.login.$invalid">
              <small class="form-text text-danger" v-if="!$v.registerAccount.login.required"> Your shop's name is required. </small>
              <small class="form-text text-danger" v-if="!$v.registerAccount.login.minLength">
                Your shop name is required to be at least 1 character.
              </small>
              <small class="form-text text-danger" v-if="!$v.registerAccount.login.maxLength">
                Your shop name cannot be longer than 50 characters.
              </small>
              <small class="form-text text-danger" v-if="!$v.registerAccount.login.pattern">
                Your shop name can only contain letters and digits.
              </small>
            </div>
          </div>
          <div class="form-group">
...
<script lang="ts" src="./addstore.component.ts"></script>
